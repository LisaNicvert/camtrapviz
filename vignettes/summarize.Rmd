...
-   `sampling_length` length of the sampling period in days (computed with the `cameraOperation` function from the `camtrapR` package).
...
-   `n_sightings` is the number of sightings of the species (corresponding to row coun in the data)
-   `n_individuals` takes into account the information from the counting column (if provided). Else, it is the same as `n_sightings`
...
If `obs_col` is included, the final table will have one more column describing type and the values will be summarized by `species_col` and `obs_col`.
